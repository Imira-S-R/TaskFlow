...
class TaskDatabase {
  ...
  Future<Database> get database async {
    ...
    _database = await _initDB('to_do_list_data.db');
    return _database!;
  }
...
CREATE TABLE $tableNotes ( 
  ...
  ${TaskFields.isDeleted} $boolType,
  ${TaskFields.isImportant} $boolType,
  ${TaskFields.dueDate} $textType,
  ${TaskFields.dueTime} $textType,
  ${TaskFields.listName} $textType
  )
''');
  }
  ...
  Future<List<Task>> readSpecificTasks(String? listName) async {

    final db = await instance.database;

    final result = await db.rawQuery('SELECT * FROM $tableNotes WHERE listName=?', ['$listName']);
